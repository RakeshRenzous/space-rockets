...
export default function FavouritesContainer({ favCb }) {
  ...
  if(categories.length === 0) {
    return <p>You haven't added any favourites yet. 
      Add more favourites by clicking on the heart icon to view them here.
    </p>;
  }
    ...
    <>
      <Box mb='4' mt='4'>
        {categories.map((category, index) => {
          ...
          >
            {`${CATEGORY_MAP[categoryName]} (${Object.keys(categoryList).length})`}
          </Button>
        })}
      </Box>

      {favourites[activeCategory] && Object.values(favourites[activeCategory]).map((categoryContent, index) => {
        return <FavouritesCard cardData={categoryContent} key={index} category={activeCategory} cbFunc={favCb}/>
      })}
    </>
  );
}
